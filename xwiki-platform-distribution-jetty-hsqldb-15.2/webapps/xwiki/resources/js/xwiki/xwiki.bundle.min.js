...
$jsontool.serialize($services.icon.renderHTML("pencil"));var h=document.createElement("span");h.className="sr-only";h.textContent=$jsontool.serialize($services.localization.render("edit"));f.appendChild(h);b.hasRenderer?f.href=window.docediturl+"?section="+a:(f.className+=" disabled",f.setAttribute("aria-disabled","true"),f.setAttribute("tabindex","-1"),f.title=$jsontool.serialize($services.localization.render("platform.core.rendering.noRendererForSectionEdit")),f.href="#"+(g.id||""));g.appendChild(f);
a++}}}}},insertCreatePageFromTemplateModalBoxes:function(d){if("xwiki/1.0"!=b.docsyntax&&"view"==b.contextaction&&b.hasEdit&&b.widgets.ModalPopup){b.widgets.CreatePagePopup=Class.create(b.widgets.ModalPopup,{initialize:function($super,e){var g=new Element("div",{"class":"modal-popup"});g.insert(e.content);$super(g,{show:{method:this.showDialog,keys:[]},close:{method:this.closeDialog,keys:["Esc"]}},{displayCloseButton:!0,verticalPosition:"center",backgroundColor:"#FFF"});this.showDialog();this.setClass("createpage-modal-popup")}});
...
"")-e.__fm_extra.getStyle("padding-left").replace(/[^0-9]/g,""))}else c(e),c(e.__fm_extra)}function d(f){"undefined"==typeof f.__fm_ghost&&(f.__fm_ghost=new Element("div"),f.__fm_ghost.hide(),f.insert({after:f.__fm_ghost}));f.__fm_ghost.clonePosition(f,{setWidth:!1})}function a(f,h,k,m){f&&(f.addClassName("floating-menu"),f.style.position="fixed",f.style.top=h+"px",f.style.left=k+"px",f.style.width=m+"px",f.__fm_ghost.show())}function c(f){f&&(f.removeClassName("floating-menu"),f.style.position="",
...
a.length).toLowerCase()==a&&f.push(k)}f.length&&(c[g]=f)}this.sInput=a;this.nInputChars=a.length;this.aSuggestions=c;for(e=0;e<sources.length;e++)a=sources[e],(c=this.aSuggestions[a.id])&&this.createList(c,a)}else{this.sInput=a;this.nInputChars=a.length;this.prepareContainer();this.latestRequest++;var m=this,n=this.latestRequest;clearTimeout(this.ajID);this.container.select(".hide-button-wrapper").invoke("hide");this.ajID=setTimeout(function(){m.doAjaxRequests(n)},this.options.delay)}return!1},doAjaxRequests:function(a,
...
g={},g[c[a]]=(new Element("div",{"class":"hide-button-wrapper"})).update(e),e.observe("click",this.clearSuggestions.bindAsEventListener(this)),this.container.insert(g);this.container.fire("xwiki:suggest:containerPrepared",{container:this.container,suggest:this});return this.container},createList:function(a,c){this._createList(a,c);this.isInMultiSourceMode&&this.resultContainer.down(".results.loading")||document.fire("xwiki:suggest:updated",{container:this.container,suggest:this})},_createList:function(a,
c){this.isActive=!0;var e=this;this.killTimeout();if(this.isInMultiSourceMode){var g=this.resultContainer.down(".results"+c.id);g.removeClassName("loading");g.down(".sourceContent").removeClassName("loading");(0<a.length||this.options.shownoresults)&&g.removeClassName("hidden");this.options.unifiedLoader&&!this.resultContainer.down(".results.loading")&&(this.options.loaderNode||this.fld).removeClassName("loading")}else g=this.resultContainer;if(0==a.length&&!this.options.shownoresults)return!1;g.down("ul")&&
g.down("ul").remove();this.container.select(".hide-button-wrapper").invoke("show");for(var f=new b.widgets.XList([],{icon:this.options.icon,classes:"suggestList",eventListeners:{click:function(n){e.setHighlightedValue(n);return!1},mouseover:function(){e.setHighlight(this.getElement())}}}),h=0,k=a.length;h<k;h++){var m=function(n){return((n||"")+"").escapeHTML()};m=(new Element("div")).insert((new Element("span",{"class":"suggestId"})).update(m(a[h].id))).insert((new Element("span",{"class":"suggestValue"})).update(m(a[h].value))).insert((new Element("span",
{"class":"suggestInfo"})).update(m(a[h].info))).insert((new Element("span",{"class":"suggestURL"})).update(m(a[h].url)));m=new b.widgets.XListItem(this.createItemDisplay(a[h],c),{containerClasses:"suggestItem "+(a[h].type||""),value:m,noHighlight:!0});f.addItem(m)}0==a.length&&f.addItem(new b.widgets.XListItem(this.options.noresults,{classes:"noSuggestion",noHighlight:!0}));g.appendChild(f.getElement());this.suggest=g;e=this;0<this.options.timeout&&(this.toID=setTimeout(function(){e.clearSuggestions()},
this.options.timeout))},createItemDisplay:function(a,c){var e=this.sInput?this.sInput.escapeHTML():this.sInput,g=((a.value||"")+"").escapeHTML();e=c.highlight?this.emphasizeMatches(e,g):g;a.hint&&(g=(a.hint+"").escapeHTML(),e+="<span class='hint'>"+g+"</span>");this.options.displayValue?(e=(new Element("div")).insert((new Element("div",{"class":"value"})).update(e)),a.info&&(g=a.info+"",(void 0===c.resultInfoHTML?this.options.resultInfoHTML:c.resultInfoHTML)||(g=g.escapeHTML()),e.insert((new Element("div",
{"class":"info"})).update("<span class='legend'>"+this.options.displayValueText+"</span>"+g)))):e=(new Element("span",{"class":"info"})).update(e);a.icon&&(a=0<=a.icon.indexOf(".")||0<=a.icon.indexOf("/")?new Element("img",{src:a.icon,"class":"icon"}):new Element("i",{"class":"icon "+a.icon}),e.insert({top:a}));return e},emphasizeMatches:function(a,c){if(!a)return c;var e=c;a=a.split(/\s+/).uniq().compact();var g=0,f={};c=0;for(var h=a.length;c<h;c++)for(var k=e.toLowerCase().indexOf(a[c].toLowerCase());0<=
k;){var m=e.substring(k,k+a[c].length),n="";a[c].length.times(function(){n+=" "});f[k]=m;e=e.substring(0,k)+n+e.substring(k+a[c].length);k=e.toLowerCase().indexOf(a[c].toLowerCase())}Object.keys(f).sortBy(function(l){return parseInt(l)}).each(function(l){var p=e.substring(0,parseInt(l)+g),q=e.substring(parseInt(l)+f[l].length+g);e=p+"<em>"+f[l]+"</em>"+q;g+=9});return e},changeHighlight:function(a){var c=this.resultContainer;if(!c)return!1;if(this.iHighlighted)if(a==Event.KEY_DOWN){var e=this.iHighlighted.next();
...
setHighlightedValue:function(a){if(this.iHighlighted&&!this.iHighlighted.hasClassName("noSuggestion")){var c=function(f){return f.textContent||f.innerText},e=this.iHighlighted.down("img.icon");c={suggest:this,id:c(this.iHighlighted.down(".suggestId")),value:c(this.iHighlighted.down(".suggestValue")),info:c(this.iHighlighted.down(".suggestInfo")),url:c(this.iHighlighted.down(".suggestURL")),icon:e?e.src:"",originalEvent:a};if(""==this.sInput&&""==this.fld.value)var g=e=c.value;else if(this.seps){a=
-1;for(e=0;e<this.seps.length;e++)this.fld.value.lastIndexOf(this.seps.charAt(e))>a&&(a=this.fld.value.lastIndexOf(this.seps.charAt(e)));-1==a?g=e=c.value:(e=this.fld.value.substring(0,a+1)+c.value,g=e.substring(a+1))}else g=e=c.value;a=Event.fire(this.fld,"xwiki:suggest:selected",Object.clone(c));!a.stopped&&(this.sInput=g,this.fld.value=e,this.fld.focus(),this.clearSuggestions(),"function"==typeof this.options.callback&&this.options.callback(Object.clone(c)),0<this.fld.id.indexOf("_suggest")&&(a=
